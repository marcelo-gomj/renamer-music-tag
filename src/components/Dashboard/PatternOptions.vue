      ...
      <div v-for="([patternKey, patterns], index) of [[], ...R.toPairs(patternReferences)]"
        ...
        <div class="space-y-0.5 py-2.5 pl-5 pr-3">
          <div v-show="currentPattern?.patternKey === patternKey"
            class="w-[3px] py-3 h-[75%] rounded-full bg-white absolute left-1"></div>
          <div class="font-bold shrink-0 pt-1 pb-3 leading-[0]">{{
            patternKey ? `Padrão ${index + 1}` : 'Todos os padrões'
          }}</div>

          <div v-if="patternKey" class="flex gap-4 shrink-0">
