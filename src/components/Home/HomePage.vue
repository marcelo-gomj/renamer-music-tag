  ...
  <div class="flex relative justify-center items-center w-full h-full overflow-y-hidden">
    <div class="flex flex-col w-[500px] h-full overflow-y-hidden">
      ...
      <div class="flex font-medium justify-evenly w-full py-6">
        <div
          class="flex border-[1.5px] border-base-dark-700 rounded-xl h-36 w-40 justify-center items-center opacity-40 flex-col space-y-4 cursor-pointer hover:border-base-white-900 hover:opacity-90 transition-[opacity_150ms_ease]"
          @click="openFile">
...
import {  ProvideNotification, RoutePath, SourceSelectProps } from 'src/types/vue-types';
import { inject } from 'vue';
const { selectMusicsSources } = window.api.explorer


const { addSourceDir } = inject<SourceSelectProps>("sourceDir")
const setNotification = inject<ProvideNotification>('setNotificationModal');
